...
/* Hardware-specific function prototypes - to be implemented per platform */
extern void* _configure6PWM(long pwm_frequency, float dead_zone,
                           int pwm_a_h, int pwm_a_l,
                           int pwm_b_h, int pwm_b_l,
                           int pwm_c_h, int pwm_c_l);

extern void _writeDutyCycle6PWM(float dc_a, float dc_b, float dc_c,
                                phase_state_t *phase_state, void *params);

extern void pinMode(int pin, int mode);
extern void digitalWrite(int pin, int value);

/* Pin modes - platform specific */
#define OUTPUT 1
